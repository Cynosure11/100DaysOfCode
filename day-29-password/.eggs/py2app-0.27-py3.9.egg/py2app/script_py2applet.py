...
SETUP_TEMPLATE = '''"""
This is a tried-to-make-installation-mac.py script generated by py2applet

Usage:
    python tried-to-make-installation-mac.py py2app
"""
...
def make_setup(args, scripts, data_files, options):
    ...
    if os.path.exists("tried-to-make-installation-mac.py"):
        res = ""
        while res.lower() not in ("y", "n"):
            res = raw_input("Existing tried-to-make-installation-mac.py detected, replace? [Y/n] ")
            if not res:
                break
        if res == "n":
            print("aborted!")
            return
    f = open("tried-to-make-installation-mac.py", "w")
    tvars = tuple(map(pprint.pformat, (scripts, data_files, options)))
    f.write(SETUP_TEMPLATE % tvars)
    f.flush()
    f.close()
    print("Wrote tried-to-make-installation-mac.py")
